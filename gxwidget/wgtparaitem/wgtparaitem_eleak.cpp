...
void wgtParaItem_eLeak::setupUi(void)
{
    ...
    ui->lblEleakNumberName->setText(/*tr("当前水浸编号：")*/langN.show[langEnum::strCurLeakNumber] + ":");
    ui->lblEleakNumberVal->setText(QString::number(m_curId));

    ui->lblEleakStaName->setText(/*tr("当前水浸工作状态：")*/langN.show[langEnum::strCurConnectStatus] + ":");
    setConnectState(1);
    createInfo();
